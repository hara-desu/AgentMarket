...
contract("Unit testing royalties", async accounts => {
  it("Should Deploy 3 Unique NFTS to 3 different accounts, with each being a child of predecessor", async () => {
    ...
    resp = await instance.mint(accounts[1], "QmNaeSB2Q7HXYG69F3ufz4DuvEApUBtpxx82AXPZnGVm2R", accZero, 0, {
      from: accounts[1],
    });
    gu.mint.push(resp.receipt.gasUsed);
    resp = await instance.mint(accounts[2], "QmbNAoDJmsEmiVTdmnCwoXjscnbnp5dw4MuCwpLiPc8XNC", accounts[1], 1, {
      from: accounts[2],
    });
    gu.mint.push(resp.receipt.gasUsed);
    resp = await instance.mint(accounts[3], "QmY3UyiYuy4cDovwPpY2ZZJATgLMTWGzDeJX7tGhSoC4nq", accounts[2], 2, {
      from: accounts[3],
    });
    gu.mint.push(resp.receipt.gasUsed);
  ...
  it("Should give control to marketplace smart contract for all tokens", async () => {
    ...
    resp = await instance.setApprovalForAll(instanceMP.address, true, { from: accounts[1] });
    gu.setApprovalForAll.push(resp.receipt.gasUsed);
    resp = await instance.setApprovalForAll(instanceMP.address, true, { from: accounts[2] });
    gu.setApprovalForAll.push(resp.receipt.gasUsed);
    resp = await instance.setApprovalForAll(instanceMP.address, true, { from: accounts[3] });
    gu.setApprovalForAll.push(resp.receipt.gasUsed);
  ...
  it("Should register workers and verifiers correctly, as well as verifying the workers", async () => {
    ...
    for (let i = 12; i < 18; i++) {
      resp = await instance.registerWorker(attestation, { from: accounts[i] });
      gu.registerWorker.push(resp.receipt.gasUsed);
    }
    //accounts 20 to 24 assigned as verifiers
    for (let j = 20; j < 25; j++) {
      resp = await instance.registerVerifier({ from: accounts[j] });
      gu.registerVerifier.push(resp.receipt.gasUsed);
    }
    //await instance.verifyWorker(accounts[2], 1, { from: accounts[2] });
    //verify workers 12 to 16
    for (let k = 12; k < 17; k++) {
      for (let z = 20; z < 25; z++) {
        // goes through the verifiers
        if (z == 22) {
          //One of the verifiers goes against the majority and votes for invalidity
    ...
    resp = await instance.verifyWorker(accounts[17], 1, { from: accounts[21] });
    gu.verifyWorker.push(resp.receipt.gasUsed);
    resp = await instance.verifyWorker(accounts[17], 1, { from: accounts[22] });
    gu.verifyWorker.push(resp.receipt.gasUsed);
    for (let w = 12; w < 18; w++) {
  ...
  it("Should initiate, process and end a model assessment appropriately", async () => {
    ...
    resp = await instance.modelVerificationReq(1, accounts[1], 3, 2, { from: accounts[1] });
    gu.modelVerificationReq.push(resp.receipt.gasUsed);
    for (let l = 12; l < 18; l++) {
      //Oracles register for tasks
      taskOracles[l - 12] = oracleCount++;
      resp = await instance.registerForTask(taskNo, { from: accounts[l] });
      gu.registerForTask.push(resp.receipt.gasUsed);
    }

    try {
      // Oracle attempts to register twice for the same task
    ...
    for (let m = 12; m < 18; m++) {
      // oracles heads submit the test results, non-head oracle are prevented from doing so
      try {
        resp = await instance.submitResult(taskNo, taskOracles[m - 12], 97, { from: accounts[m] });
        gu.submitResult.push(resp.receipt.gasUsed);
      } catch (e) {
    ...
    let taskOracles2 = [];
    oracleCount = 0;
    respt = await instance.modelVerificationReq(3, accounts[3], 3, 2, { from: accounts[3] });
    gu.modelVerificationReq.push(resp.receipt.gasUsed);
    for (let l = 17; l >= 12; l--) {
    ...
    for (let m = 12; m < 18; m++) {
      try {
        resp = await instance.submitResult(taskNo, taskOracles2[m - 12], 97, { from: accounts[m] });
        gu.submitResult.push(resp.receipt.gasUsed);
      } catch (e) {
  ...
  it("Should start auction correctly and prevent misbehaviour", async () => {
    ...
    resp = await instance1.registerOracle({ from: accounts[6] });
    gu.registerOracle.push(resp.receipt.gasUsed);
    ...
    resp = await instance1.startAuction(accounts[3], 3, 1662797637, { from: accounts[3] });
    gu.startAuction.push(resp.receipt.gasUsed);
  ...
  it("Should ensure correct bidding functionality", async () => {
    const instance1 = await AIMP_SC.deployed();
    var resp;
    resp = await instance1.bidOnAuction(3, { from: accounts[8], value: 90000000000000000 });
    gu.bidOnAuction.push(resp.receipt.gasUsed);
    resp = await instance1.bidOnAuction(3, { from: accounts[9], value: 95000000000000000 });
    gu.bidOnAuction.push(resp.receipt.gasUsed);
    try {
  ...
  it("Should ensure correct withdrawing functionality", async () => {
    ...
    resp = await instance1.withdrawBid({ from: accounts[8] });
    gu.withdrawBid.push(resp.receipt.gasUsed);
  });
  it("Should ensure correct end auction functionality", async () => {
    ...
    resp = await instance1.endAuction(3, { from: accounts[3] });
    gu.endAuction.push(resp.receipt.gasUsed);
  });
  it("Should successfully assign shares, transfer token and asset to buyer", async () => {
    const instance1 = await AIMP_SC.deployed();
    const instance2 = await AINFT.deployed();
    var resp;
    resp = await instance2.registerREP({ from: accounts[18] });
    gu.registerREP.push(resp.receipt.gasUsed);
    resp = await instance2.registerREP({ from: accounts[19] });
    gu.registerREP.push(resp.receipt.gasUsed);
    ...
    for (let i = 0; i < length; i++) {
      payment = (amount * (share * (i + 1)) * (((11 - length) * 100) / 2)) / 1000000;
      amount -= payment;
    }
    ...
    resp = await instance2.sendAsset(
      3,
      "QmNaeSB2Q7HXYG69F3ufz4DuvEApUBtpxx82AXPZnGVm2K",
      "470adf558bde5b94e0141de306c293e26d6b140d4230525a51864d0b8982d9e1",
      "0774c27cc64750d3c11dd3e22cfea7cd397278923ec29e4845d68da0fcb185e1",
      { from: accounts[18] },
    );
    gu.sendAsset.push(resp.receipt.gasUsed);
    resp = await instance2.sendAsset(
      3,
      "QmPaeSB2Q7HXYG69F3ufz4DuvEApUBtpxx82AXPZnGVm2K",
      "770adf558bde5b94e0141de306c293e26d6b140d4230525a51864d0b8982d9e1",
      "2774c27cc64750d3c11dd3e22cfea7cd397278923ec29e4845d68da0fcb185e1",
      { from: accounts[19] },
    );
    gu.sendAsset.push(resp.receipt.gasUsed);
    resp = await instance2.sendAsset(3, "", "", "", { from: accounts[9] });
    gu.sendAsset.push(resp.receipt.gasUsed);
    resp = await instance1.transferAndRecieveFunds(3, { from: accounts[3] });
    gu.transferAndRecieveFunds.push(resp.receipt.gasUsed);
  });
  it("Should allow eligible users to withdraw funds correclty", async () => {
    ...
    resp = await instance1.withdrawFunds({ from: accounts[1] });
    gu.withdrawFunds.push(resp.receipt.gasUsed);
    resp = await instance1.withdrawFunds({ from: accounts[2] });
    gu.withdrawFunds.push(resp.receipt.gasUsed);
    resp = await instance1.withdrawFunds({ from: accounts[3] });
    gu.withdrawFunds.push(resp.receipt.gasUsed);
