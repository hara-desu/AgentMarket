...
const NftInteract = () => {
  ...
  const handleUploadAndRegister = async () => {
    ...
    try {
      ...
      if (!fileResponse.success) {
        console.error("Failed to upload Dockerfile:");
        return;
      }
      if ('pinataUrl' in fileResponse) {
        const metadata = { name, description, docker: fileResponse.pinataUrl};
        const jsonResponse = await pinJsonToIpfs(metadata);
        if (!jsonResponse.success) {
          console.error("Failed to upload metadata:", jsonResponse);
          return;
        }
        if ('pinataUrl' in jsonResponse) {
  ...
  const handleUploadAndUpdate = async () => {
    ...
    try {
      const fileResponse = await pinFileToIpfs(dockerFile);
      if (!fileResponse.success) {
        console.error("Failed to upload Dockerfile:", fileResponse);
        return;
      }
      if ("pinataUrl" in fileResponse) {
      const metadata = { name, description, docker: fileResponse.pinataUrl };
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
          <div className="bg-white p-10 rounded-lg shadow-lg w-96">
            ...
            <div className="flex justify-between mb-4">
              ...
              <button onClick={() => setModalType(null)} className="text-gray-500 hover:text-gray-800">
                &times;
              </button>
            </div>
